...
from scientific_agent.literature_policy import decide_literature_ingest_policy, render_literature_ingest_digest
...
class WorkflowRuntime:
    def __init__(self, root: str | Path) -> None:
        ...
        self._threads_path = self.root / ".scientific_agent_service" / "threads.json"
        self._group_roles_path = self.root / ".scientific_agent_service" / "group_roles.json"
        self._memberships_path = self.root / ".scientific_agent_service" / "memberships.json"
        self._experiments_root = self.root / ".scientific_agent_service" / "experiments"
        ...
        self._research_graph_registry = ResearchGraphRegistry(self.root / ".scientific_agent_graph")
        self._event_ledger = ResearchEventLedger(self.root / ".scientific_agent_events")
    ...
    async def _run_workflow(
        ...
        *,
        ...
        try:
            skill_runtime = SkillRuntime(load_skills(self.root / "scientific_agent" / "skills_builtin"))
            model_registry = ModelRegistry(default_model="gpt-5")
            config_path = self.root / "scientific_agent_config.json"
