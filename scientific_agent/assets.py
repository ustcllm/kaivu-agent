...
def build_unified_asset_summary(
    *,
    ...
    scientific_agent_evaluation_harness_summary: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    if scientific_agent_evaluation_harness_summary:
        harness_id = (
            str(scientific_agent_evaluation_harness_summary.get("harness_id", "")).strip()
            or f"scientific-agent-evaluation-harness::{_slugify(topic)}"
        )
        add(
            ScientificAsset(
                asset_id=harness_id,
                asset_type="scientific_agent_evaluation_harness",
                label=f"scientific agent evaluation harness for {topic}",
                role="system_quality_evaluation",
                source_system="evaluation_harness",
                status=str(scientific_agent_evaluation_harness_summary.get("release_state", "not_ready")).strip(),
                governance_status="required_before_release",
                project_id=project_id,
                topic=topic,
                evidence_refs=[
                    str(axis.get("axis_id", "")).strip()
                    for axis in scientific_agent_evaluation_harness_summary.get("axes", [])
                    if isinstance(axis, dict) and str(axis.get("axis_id", "")).strip()
                ],
                metadata=scientific_agent_evaluation_harness_summary,
            )
        )
