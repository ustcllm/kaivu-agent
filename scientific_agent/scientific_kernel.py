...
def build_research_state_machine_summary(
    *,
    ...
    evaluation = research_state.get("scientific_agent_evaluation_harness_summary", {})
...
def build_benchmark_dataset_summary(root: str | Path | None = None) -> dict[str, Any]:
    ...
    return {
        "dataset_id": str(manifest.get("dataset_id", "scientific-agent-benchmark")).strip(),
        "version": str(manifest.get("version", "0.1.0")).strip(),
        "task_family": str(manifest.get("task_family", "scientific_agent_kernel")).strip(),
        "case_count": len(cases),
...
def build_benchmark_regression_suite_from_trajectory(
    *,
    ...
    context = {
        ...
        "scientific_agent_evaluation_harness_summary": evaluation,
        "run_manifest": {
...
def load_builtin_benchmark_dataset(root: str | Path | None = None) -> dict[str, Any]:
    ...
    manifest = (
        ...
        if manifest_path.exists()
            ...
            "dataset_id": "scientific-agent-benchmark",
            "version": "0.1.0",
            "task_family": "scientific_agent_kernel",
