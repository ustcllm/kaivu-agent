...
class MemoryManager:
    def __init__(self, root: str | Path, *, agent_namespace: str | None = None) -> None:
        ...
        self.graph_registry = ResearchGraphRegistry(self.root / ".scientific_agent_graph")
    ...
    def _discover_static_memory_files(self) -> list[Path]:
        candidates = [self.root / "CLAUDE.md", self.root / ".scientific_agent" / "CLAUDE.md"]
        rules_dir = self.root / ".scientific_agent" / "rules"
        if rules_dir.exists():
