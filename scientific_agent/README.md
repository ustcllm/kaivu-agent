...
This is a Python scientific-agent framework distilled from the core agent loop ideas in `cc`, then extended for research workflows.
...
The framework now mirrors the most useful memory layers from `cc`:

- instruction memory:
  reads `CLAUDE.md`, `.scientific_agent/CLAUDE.md`, and `.scientific_agent/rules/*.md`
...
- `SCIENTIFIC_AGENT_MODEL`
- `SCIENTIFIC_AGENT_TOPIC`
- `SCIENTIFIC_AGENT_REPORT_PATH`
- `SCIENTIFIC_AGENT_DYNAMIC_ROUTING`
- `SCIENTIFIC_AGENT_MCP_CONFIG`
- `SCIENTIFIC_AGENT_MODEL_CONFIG`

Per-agent model policy can now be loaded from `scientific_agent_config.json` at the workspace root, or from a custom path via `SCIENTIFIC_AGENT_MODEL_CONFIG`.
...
- `SCIENTIFIC_AGENT_CRITIC_MODEL`
- `SCIENTIFIC_AGENT_CRITIC_REASONING`
- `SCIENTIFIC_AGENT_REPORT_WRITER_MODEL`

Local stub demo:

```bash
python C:/Users/liand/Documents/agent/scientific_agent/stub_demo.py
```
...
- bundled skills are loaded from `scientific_agent/skills_builtin`
