...
if __package__ in (None, ""):
    ...
    from kaivu.data_tools import BasicStatsTool, PlotCsvTool, ReadTableTool
    from kaivu.literature_tools import ArxivSearchTool, CrossrefSearchTool, PubMedSearchTool, ResolveCitationTool
    from kaivu.memory import MemoryManager
    from kaivu.mcp import MCPRegistry, MCPServerConfig
    from kaivu.permissions import PermissionPolicy
    from kaivu.skills import SkillRuntime, load_skills
    from kaivu.model_registry import ModelRegistry
...
async def main() -> None:
        ...
        print(
            "\n".join(
                [
                    "Kaivu（开物） demo",
                    "",
                    "Usage: python -m kaivu.demo [research topic]",
                    "   or: python scripts/run_demo.py [research topic]",
                    "",
                    "Environment:",
                    "  OPENAI_API_KEY is required for the real model demo.",
                    "  KAIVU_MODEL optionally overrides the default model.",
                    "  KAIVU_MODEL_CONFIG optionally points to per-agent model config JSON.",
                    "  KAIVU_MCP_CONFIG optionally points to MCP server config JSON.",
                ]
            )
        )
        return
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise SystemExit(
            ...
            "  python C:\\Users\\liand\\Documents\\agent\\kaivu-agent\\scripts\\run_demo.py"
        )

    skills = load_skills(root / "kaivu" / "skills_builtin")
    ...
    model_registry = ModelRegistry(
        default_model=os.getenv("KAIVU_MODEL", "gpt-5"),
    ...
    workflow = ScientificWorkflow(
        ...
        dynamic_routing=os.getenv("KAIVU_DYNAMIC_ROUTING", "1") != "0",
...
def _load_mcp_configs_from_env() -> list[MCPServerConfig]:
    config_path = os.getenv("KAIVU_MCP_CONFIG")
    if not config_path:
        return []
...
def _resolve_model_config_path(root: Path) -> Path | None:
    explicit = os.getenv("KAIVU_MODEL_CONFIG")
    if explicit:
        path = Path(explicit).resolve()
        return path if path.exists() else None
    default_path = root / "config" / "agents.json"
